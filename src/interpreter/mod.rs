...
pub struct Interpreter {
    ...
    processes: Mutex<Vec<(Uuid, JoinHandle<Result<RuntimeValue, RuntimeError>>)>>,
}

impl Interpreter {
    pub fn new() -> Arc<Interpreter> {
    ...
    pub fn spawn_process(self: &Arc<Self>, new_env: EnvPtr, tree: AST) -> Uuid {
        ...
        let mut process = Process::new(uuid, runtime, new_env, mailbox);
        ...
        loop {
            ...
            match handle_opt {
                Some((uuid, handle)) => {
                    ...
                    //     Err(join_err) => eprintln!("Process {uuid} panicked: {join_err:?}"),
                    // }
                }
                None => break, // no more processes alive
            }
        }
    }
